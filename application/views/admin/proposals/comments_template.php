...
<div class="col-md-12 comment-item" data-commentid="<?php echo e($comment['id']); ?>">
    ...
    <a href="#" class="pull-right" onclick="remove_proposal_comment(<?php echo e($comment['id']); ?>); return false;">
        <i class="fa fa-times text-danger"></i>
        <a href="#" class="pull-right mright5"
            onclick="toggle_proposal_comment_edit(<?php echo e($comment['id']); ?>);return false;"><i
    ...
    <div class="media-body">
        ...
        <div data-proposal-comment="<?php echo e($comment['id']); ?>" class="tw-mt-3">
            <?php echo process_text_content_for_display($comment['content']); ?>
        </div>
        <div data-proposal-comment-edit-textarea="<?php echo e($comment['id']); ?>" class="hide tw-mt-3">
            <?php echo render_textarea('comment-content', '', $comment['content']); ?>
            <?php if ($comment['staffid'] == get_staff_user_id() || is_admin()) { ?>
            <div class="text-right">
                <button type="button" class="btn btn-default"
                    onclick="toggle_proposal_comment_edit(<?php echo e($comment['id']); ?>);return false;"><?php echo _l('cancel'); ?></button>
                <button type="button" class="btn btn-primary"
                    onclick="edit_proposal_comment(<?php echo e($comment['id']); ?>);"><?php echo _l('update_comment'); ?></button>
            </div>
            <?php } ?>
        </div>
    </div>
